...
for (k in 1:number_of_multiple_imputations) {
  print(k)
  dat_long_k <- dat_long %>%
    left_join(dat_long_with_posterior%>%
                dplyr::select(c("id", "age", paste0("post", r[k]))), by = c("id", "age"))%>%
    group_by(id) %>%          
    ...
    mutate(t_minus_t_dropout_pos = (age - age_dropout)*(age - age_dropout>0),
           predicted_EM_delta = ifelse(age_dropout<60, !!as.name(paste0("post", r[k])), !!as.name(paste0("post", r[k])) + delta*t_minus_t_dropout_pos), # predicted by a pattern-mixture + delta*(t-t_ifi)+
