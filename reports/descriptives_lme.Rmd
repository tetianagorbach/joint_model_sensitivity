...
```{r, eval = F, include = F}
print(xtable(dat_long_with_missing%>%
  filter(!is.na(EM))%>%
...
print(
  xtable(
    rbind(summary_by_missing_data_pattern, 
...
summary_by_missing_data_pattern <-
  ...
  summarise(
    ...
    n_censored_dem_eval = paste0(sum(age_censor == age_dementia_eval, na.rm = T),
                                  " (", round(sum(age_censor == age_dementia_eval, na.rm = T)/n * 100, digits = 0), ")"),
    n_censored_death = paste0(sum(age_censor ==  age_death, na.rm = T),
                               " (", round(sum(age_censor == age_death, na.rm = T)/n * 100, digits = 0), ")"),
    n_censored_last_obs = paste0(sum(age_censor == age_last_obs + 0.003, na.rm=T),
...
pdf(file = "Figure_mean_EM_per_pattern.pdf",   # The directory you want to save the file in
...
dat_long%>%
  ...
  summarise(
    ...
    mean_memory = mean(EM))%>%
  ungroup()%>%
  ggplot(aes(x = mean_age_cohort, y = mean_memory,  group = missing_data_pattern, 
  ...
  scale_linetype_manual(values=c("solid", "longdash","twodash","dotdash","dotted", "dotted"),
...
```{r lme models fit}
dat_long <- dat_long%>%
  mutate(t = age_sc20 - mean_age_sc20)
lme_fit_1 <- nlme::lme(EM_sc ~ t + 
  sex + educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ age_sc20 | id, data = dat_long, method = "ML"
)

lme_fit_2 <- nlme::lme(EM_sc ~ poly(t, 2, raw = TRUE) +
  sex + educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ t | id, data = dat_long, method = "ML"
)

lme_fit_3 <- nlme::lme(EM_sc ~ poly(t, 3, raw = TRUE) +
  sex + educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ t | id, data = dat_long, method = "ML"
)

lme_fit_4 <- nlme::lme(EM_sc ~ poly(t, 4, raw = TRUE) +
  sex + educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ t| id, data = dat_long, method = "ML"
)

lme_fit_5 <- nlme::lme(EM_sc ~ poly(t, 5, raw = TRUE) +
  sex +educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ t | id, data = dat_long, method = "ML"
)

lme_fit_ns2 <- nlme::lme(EM_sc ~ ns(t, df = 2) +
  sex + educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ t | id, data = dat_long, method = "ML"
)

lme_fit_ns3 <- nlme::lme(EM_sc ~ ns(t, df = 3) +
  sex + educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ t | id, data = dat_long, method = "ML"
)

lme_fit_ns4 <- nlme::lme(EM_sc ~ ns(t, df = 4) +
  sex + educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ t | id, data = dat_long, method = "ML"
)

lme_fit_ns5 <- nlme::lme(EM_sc ~ ns(t, df = 5) +
  sex + educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ t | id, data = dat_long, method = "ML"
)
lme_fit_ns6 <- nlme::lme(EM_sc ~ ns(t, df = 6) +
  sex + educ_imp_sc + ind_enrol + I(ind_enrol * t) + cohort_sc,
random = ~ t| id, data = dat_long, method = "ML"
)
```
...
anova(lme_fit_1, lme_fit_2, lme_fit_3,  lme_fit_4, lme_fit_5, 
...
pdf(file = "Figure_mean_predictions.pdf",   # The directory you want to save the file in
   width = 5,
   height = 3)# The width of the plot in inches)
ggplot() +
  geom_point(aes(x = (age_sc20)*100 +20, y = EM_sc, group = id), dat_long, size = 0.1, col = "grey") +
  geom_line(aes(x = (age_sc20)*100 +20 , y = EM_sc, group = id), dat_long, size = 0.1, col = "grey") +
  geom_line(aes(x = (t + 0.4214415)*100 + 20 , y = predict_lme_fit3$fit, group = 1), colour = "#FFC20A", size = 0.5)  + 
  geom_line(aes(x = (t + 0.4214415)*100 + 20 , y = predict_lme_fitns3$fit, group = 1), colour = "#0C7BDC", size = 0.5)  +
  geom_ribbon(aes(x = (t + 0.4214415)*100 + 20  ,
                  ymin = predict_lme_fit3$fit - qnorm(0.975)*predict_lme_fit3$se.fit,
                  ymax = predict_lme_fit3$fit + qnorm(0.975)*predict_lme_fit3$se.fit), fill = "#FFC20A", alpha = 0.4) +
geom_ribbon(aes(x = (t + 0.4214415)*100 + 20,
                  ymin = predict_lme_fitns3$fit - qnorm(0.975)*predict_lme_fitns3$se.fit,
                  ymax = predict_lme_fitns3$fit + qnorm(0.975)*predict_lme_fitns3$se.fit), fill = "#0C7BDC", alpha = 0.4) +
  theme_classic() +
...
pdf(file = "Figure_difference_lme.pdf",   # The directory you want to save the file in
   width = 5,
   height = 3)
